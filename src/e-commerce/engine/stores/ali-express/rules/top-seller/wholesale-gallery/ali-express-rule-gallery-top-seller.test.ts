...
import { AliExpressRuleGalleryTopSeller } from "./ali-express-rule-gallery-top-seller"

describe("Ali Express Rule Top Seller", () => {
	const aliExpressRuleTopSeller: AliExpressRuleGalleryTopSeller = new AliExpressRuleGalleryTopSeller()
	beforeAll(() => {
	...
	it("rule evaluation work correctly", async () => {
		...
		const expected: IRuleResult = {
			...
			name: rules_name.TOP_SELLER_WHOLESALE_GALLERY,
			value: 0,
			weight: 0,
			isValidRule: true,
			tooltipSummary: {
