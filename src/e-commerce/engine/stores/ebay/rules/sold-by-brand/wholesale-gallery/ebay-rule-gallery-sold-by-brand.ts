...
export class EbayRuleGallerySoldByBrand extends Rule {
	constructor() {
		super(rules_name.SOLD_BY_BRAND_WHOLESALE_GALLERY, WEIGHT.NONE)
	}
	...
	public async evaluate(product: IProduct, siteDomSelector: SiteDomSelector): Promise<IRuleResult> {
		const res = await getRuleSoldByBrandResultValue(
			product,
			siteDomSelector.domSelector.wholesaleGalleryPageItemSoldByBrand,
			this.weight,
			this.name
		)
