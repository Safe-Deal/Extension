...
export function initPegasusTransport(): void {
  ...
  const messageRuntime = createMessageRuntime("devtools", async (message) => port.postMessage(message));

  port.onMessage((packet) => internalPacketTypeRouter(packet, { eventRuntime, messageRuntime }));

  const eventRuntime = createBroadcastEventRuntime("devtools", async (event) => {
