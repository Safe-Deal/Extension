...
const argv = yargs(hideBin(process.argv))
    ...
    .parseSync();
const logAndExitIfNeeded = (message, isWarning = false) => {
...
for (const directory of directories) {
    if (!supportedLocales.includes(directory.name)) {
        logAndExitIfNeeded(`Error: Invalid locale directory '${directory.name}' detected in '${path.join(localesDirPath, directory.name)}'. Please check the locale directory name.`);
    }
}
const baseLangFilePath = path.join(localesDirPath, argv.baseLang, "messages.json");
let baseLangMessages = {};
try {
    ...
    try {
        ...
        for (const key of Object.keys(baseLangMessages)) {
            if (!otherLocaleKeysSet.has(key)) {
                logAndExitIfNeeded(`Key "${key}" in base language \`${argv.baseLang}\` does not exist in \`${dir}\``, true);
            }
        }
        for (const key of otherLocaleKeysSet) {
            if (!Object.hasOwn(baseLangMessages, key)) {
                logAndExitIfNeeded(`Key "${key}" in \`${dir}\` does not exist in base language \`${argv.baseLang}\``);
            }
        }
    }
    catch (err) {
