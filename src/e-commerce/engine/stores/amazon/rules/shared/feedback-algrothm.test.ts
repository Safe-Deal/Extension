...
import { calculateAmazonItemDetailsFeedbackAlgorithm } from "./feedback-algrothm"
...
describe("Amazon - Rule product feedback algorithm", () => {
	describe("Positive scenario", () => {
		it("great product??", () => {
			...
			expect(calculateAmazonItemDetailsFeedbackAlgorithm(productRatingValue, productNumberOfReviewsValue)).toBe(
		...
		it("should return rule value 1 when there is no ratings, reviews and orders number", () => {
			expect(calculateAmazonItemDetailsFeedbackAlgorithm()).toBe(RULE_VALUE.RULE_VAL_1)
		})

		it("should return value of 1 for invalid product - tiny amount", () => {
			const rating = 1
			const reviews = 1
			expect(calculateAmazonItemDetailsFeedbackAlgorithm(rating, reviews)).toBe(RULE_VALUE.RULE_VAL_1)
		})
		...
		it("excellent product", () => {
			const rating = 5
			const reviews = 50
			expect(calculateAmazonItemDetailsFeedbackAlgorithm(rating, reviews)).toBe(RULE_VALUE.RULE_VAL_8)
		})
