...
import { AlibabaSiteUtils } from "../../../../supplier/utils/alibaba-site-utils";
...
export class SiteUtil {
  ...
  public static getStore(url = SiteMetadata.getURL()): ProductStore {
    const storeChecks = [
      { check: AliExpressSiteUtils.isAliExpressRussianSite, store: ProductStore.ALI_EXPRESS_RUSSIA },
      { check: AliExpressSiteUtils.isAliExpressSite, store: ProductStore.ALI_EXPRESS },
      { check: AmazonSiteUtils.isAmazonSite, store: ProductStore.AMAZON },
      { check: EbaySiteUtils.isEbaySite, store: ProductStore.EBAY },
      { check: AlibabaSiteUtils.isAlibabaSite, store: ProductStore.ALIBABA }
    ];

    for (const { check, store } of storeChecks) {
      if (check(url)) {
        return store;
      }
    }
  ...
  public static isAlibabaSupplierSite(url = SiteMetadata.getURL()): boolean {
    return AlibabaSiteUtils.isAlibabaSupplier(url);
  }
}
