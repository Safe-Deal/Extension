...
describe("rule product feedback algorithm", () => {
	...
	it("should return value 7 when store positive feedback between 95 and 98", () => {
		...
		expect(calculateStorePositiveFeedbackValueAlgorithm(storePositiveFeedback)).toBe(RULE_VALUE.RULE_VAL_7)
	})
	...
	it("should return value 2 when store positive feedback lower then 89", () => {
		...
		expect(calculateStorePositiveFeedbackValueAlgorithm(storePositiveFeedback)).toBe(RULE_VALUE.RULE_VAL_5)
	})

	it("should return value -10 when store positive feedback lower then 90", () => {
		const storePositiveFeedback = 79
		expect(calculateStorePositiveFeedbackValueAlgorithm(storePositiveFeedback)).toBe(RULE_VALUE.RULE_VAL_NEGATIVE)
	})
})
