...
const STORES_HANDLERS = {
  ...
  [ProductStore.ALIBABA]: () => {
    const titleElement = querySelector(["div.module_title > div > h1"], document);
    if (titleElement) {
      const name = titleElement.textContent.trim();
      return trimProductName(name);
    }
    handleError(new Error("Product name not found on Alibaba page"), "Research:: useProductName:: getProductName");
    return "";
  }
};
