...
import { IRPCMessaging } from "./IRPCMessaging";
...
>(serviceName: string, service: TService): void {
  ...
  const { onMessage } = definePegasusMessageBus<IRPCMessaging>();
  ...
  onMessage(messageKey, ({ data, ...message }) => {
    if (
      ...
      throw new Error(
        `Invalid message received for pegasus-rpc-service "${serviceName}": ${JSON.stringify(data, undefined, 2)}`
      );
    }

    const path: string | null = data.path == null || typeof data.path !== "string" ? null : data.path;
    ...
    if (typeof serviceCb !== "function") {
      throw new Error(
        `Invalid message received for pegasus-rpc-service "${serviceName}": ${
          path != null ? `Can't find method "${path}` : "Expected service to be a function"
        }"`
      );
    }
