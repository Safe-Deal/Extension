...
import { PegasusStore } from "@utils/pegasus/store";
import getConfiguration from "./getConfiguration";
import { ZustandAction } from "./types";

const storeRegistry = new WeakMap<StoreApi<unknown>, { [key: string]: Promise<void> }>();

export async function pegasusZustandStoreReady<S>(storeName: string, store: StoreApi<S>): Promise<StoreApi<S>> {
  ...
  const callback = (state: S, oldState: S) => {
    proxyStore
      ...
      .catch((err) => {
        if (err instanceof Error && err.message === "Extension context invalidated.") {
          console.warn("Webext-Zustand: Reloading page as we lost connection to background script...");
