...
export class Remote {
  ...
  public static async getJson(url: string, isCached = true): Promise<DownloadResult> {
    const cachingKey = this.getCachingKey(url);
    return this.fetchData(isCached, cachingKey, () => getFetchJson(url));
  }
  ...
  public static async postJson(url: string, json: object, ignoreCache: boolean = false): Promise<DownloadResult> {
    const cachingKey = this.postCachingKey(url, json);
    return this.fetchData(!ignoreCache, cachingKey, () => postFetchJson(url, json));
  }
