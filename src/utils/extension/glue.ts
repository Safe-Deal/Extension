...
const WORKER_MESSAGE_TYPE = Object.freeze({
  ...
  WHOLESALE_WAREHOUSE: "wholesale_warehouse"
});
...
export const WHOLESALE_WAREHOUSE_GLUE = {
  worker(onProduct: BackgroundListener) {
    addWorkerHandler(WORKER_MESSAGE_TYPE.WHOLESALE_WAREHOUSE, onProduct);
  },
  client(onClient: ContentListener) {
    registerClientListener(WORKER_MESSAGE_TYPE.WHOLESALE_WAREHOUSE, onClient);
  },
  send(product: any) {
    sendMessage(WORKER_MESSAGE_TYPE.WHOLESALE_WAREHOUSE, product);
  }
};
