import { getRPCService } from "@utils/pegasus/rpc";
...
export class PegasusStore<S, A extends PegasusStoreAction = PegasusStoreAnyAction> implements IPegasusStore<S, A> {
  ...
  constructor({
    ...
    onBroadcastEvent<string>(`pegasusStore/${portName}/${MessageType.STATE}`, (event) => {
    ...
    onBroadcastEvent<string>(`pegasusStore/${portName}/${MessageType.PATCH_STATE}`, (event) => {
  ...
  async dispatch(data: A): Promise<A> {
    return this.deserializer(await this.bridge.dispatch(this.serializer(data))) as A;
  }
}
